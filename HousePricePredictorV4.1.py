...
def ReadInTrainingData():
    ...
    with open("GrLivAreaLotAreaNoOutliers.csv") as fd: #change file name
...
def ReadInTestData():
    ...
    with open("TestGrLivAreaLotArea.csv") as fd: #change file name
        rd = csv.reader(fd, delimiter=",")
        for row in rd:
...
def CalcRegressionFormula(Y,X1,X2): #Improve variable names
    """
...
def WriteToFile(PredictedY,IdList):
    ...
    """
    with open('PredictedHousePricesV5.csv', mode = 'w') as csvfile: #change file name
...
OutputRegressionFormula(B_1,B_2,B_0) # remove this in final version
